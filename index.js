...
app.post('/webhook', async (req, res) => {
    try {
        ...
        const tokenBurns = transaction.tokenBurns || []; // Check for burn events if provided by Helius

        // Check for token transfers to burn addresses
        for (const transfer of tokenTransfers) {
            if (!burnDetected && BURN_ADDRESSES.some(address => 
                ...
                let attempt = 0;
                const maxAttempts = 1; // Limit to one initial attempt
                ...
                if (attempt === maxAttempts && !burnDetected) {
        ...
        // Check for programmatic burns (if supported by Helius)
        if (!burnDetected && tokenBurns.length > 0) {
            for (const burn of tokenBurns) {
                if (burn.mint === CONTRACT_ADDRESS) {
                    const burnAmount = burn.amount / 10 ** burn.decimals; // Adjust based on Helius data structure
                    const gifUrl = 'https://media.giphy.com/media/3o7TKTDn976rzVgDf2/giphy.gif';
                    ...
                    while (attempt < maxAttempts) {
                        try {
                            console.log(`Attempting to send GIF and message for program burn (Attempt ${attempt + 1})...`);
                            await axios.post(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendAnimation`, {
                                chat_id: TELEGRAM_CHAT_ID,
                                animation: gifUrl
                            });
                    ...
                    if (attempt === maxAttempts && !burnDetected) {
                        console.error('Failed to send GIF and message for program burn after maximum attempts');
                    }
                    break;
                }
            }
