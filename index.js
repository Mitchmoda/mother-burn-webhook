...
const CONTRACT_ADDRESS = '3S8qX1MsMqRbiwKg2cQyx7nis1oHMgaCuc9c4VfvVdPN'; // Updated to correct address
        ...
        for (const transfer of tokenTransfers) {
            if (!burnProcessed && BURN_ADDRESSES.some(address => 
                        ...
                        while (messageAttempt < maxMessageAttempts) {
                            try {
                                ...
                                if (error.response && error.response.status === 429 && error.response.data.parameters.retry_after) {
                                    ...
                                    console.error('Message send error:', new Date().toISOString(), ':', error.message, error.response ? error.response.data : 'No response data');
                                    throw error;
                                }
                            }
                        }
                        if (messageAttempt === maxMessageAttempts && !burnProcessed) {
                            console.error('Failed to send message after maximum attempts at:', new Date().toISOString());
                        }
                    } catch (error) {
                ...
                if (burnProcessed) {
                    ...
                    console.error('Failed to process burn after maximum attempts at:', new Date().toISOString());
                }
                break;
            }
        }
            ...
            for (const burn of tokenBurns) {
                if (burn.mint === CONTRACT_ADDRESS) {
                            ...
                            while (messageAttempt < maxMessageAttempts) {
                                try {
                                    ...
                                    if (error.response && error.response.status === 429 && error.response.data.parameters.retry_after) {
                                        ...
                                        console.error('Message send error for program burn:', new Date().toISOString(), ':', error.message, error.response ? error.response.data : 'No response data');
                                        throw error;
                                    }
                                }
                            }
                            if (messageAttempt === maxMessageAttempts && !burnProcessed) {
                                console.error('Failed to send message for program burn after maximum attempts at:', new Date().toISOString());
                            }
                        } catch (error) {
                    ...
                    if (burnProcessed) {
                        ...
                        console.error('Failed to process program burn after maximum attempts at:', new Date().toISOString());
                    }
                    break;
                }
            }
